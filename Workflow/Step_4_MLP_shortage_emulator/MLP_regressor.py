...
training_data_path = "C:/Users/fere556/Desktop/HPC_testing/output_sobol_13312/" # path to Artes model outputs of water shortage, generated in Step 2 and processed in Step 3
...
path = "C:/Users/fere556/Desktop/N3_LA_Water_Modeling-main/"
...
synthetic_scenario = 'REPO_mwd_import_lower_range'
synthetic_scenario_names = []
synthetic_path = "C:/Users/fere556/Desktop/REPO/" # " # OneDrive - PNNL/Documents/Artes/Artes_paper/LA_Sensitivity_Analysis_Repo/GitHub/Step_4_MLP_predictor/" # path for synthetic sample ensembles 
os.makedirs(synthetic_path + synthetic_scenario, exist_ok=True) # create folder for synthetic scenario ensemble 

if generate_synthetic:
    path_params = "C:/Users/fere556/Desktop/N3_LA_Water_Modeling-main/Input_files/"
    parameter_df = pd.read_csv(path_params + "SALib_parameters_HPC_test.csv") # csv with parameters and bounds SALib_parameters_Exp1 or SALib_parameters_Exp2_no_demand.csv"
...
# X_syn = shortage_comparison.iloc[:,:].values # input features for synthetic prediction create inner loop for convergence plot 
...
for i in range(len(column_labels)): # Iterate over columns. New MLP trained for each column.  
    ...
    # Plots of pred vs test
    # Plot Artes results vs emulated 
    # plt.figure()
    # plt.title("Total shortage: Artes vs emulator for " + column_labels[i])
    # plt.scatter(y_train_test_df.test, y_train_test_df.predict, s = 4, alpha = 0.05)
    # plt.ylabel('MLP pred')
    # plt.xlabel('Artes truth')
...
#%% Plot metrics distributions & Artes vs MLP scatter plots - Figure 5 

# ADD load MLP performance metrics csv
...
# Box plot with NO FILL and styled lines
bp = ax.boxplot(
    metrics.iloc[3,:97],
    vert=True,
...
# Titles/labels
#ax.set_title('Distribution of Example Data - Box and Whisker Plot', fontsize=11)
...
# Save a high-resolution image (DPI can be set here too)
fig.savefig('boxplot_no_fill_300dpi.png', dpi=300, bbox_inches='tight')
plt.figure(figsize=(2, 4), dpi=300)
plt.boxplot(metrics.iloc[0,:97], vert=True, patch_artist=True, color = 'k')
#plt.title('Distribution of Example Data - Box and Whisker Plot')
plt.ylabel('R2 Values')
plt.show()

plt.figure(figsize=(6, 4))
plt.boxplot(metrics.iloc[3,:97], vert=True, patch_artist=True)
plt.title('Distribution of Example Data - Box and Whisker Plot')
plt.ylabel('Values')
plt.show()

#%% Add Sobol analysis here 

data_path = (synthetic_path + synthetic_scenario + "/")
os.chdir(data_path)

parameter_df = pd.read_csv("C:/Users/fere556/Desktop/N3_LA_Water_Modeling-main/Input_files/SALib_parameters_HPC_test.csv")  # csv with parameters and bounds SALib_parameters_Exp1 or SALib_parameters_Exp2_no_demand.csv"
problem = {'num_vars': len(parameter_df.Parameter),
           'names': list(parameter_df.Parameter.values),
           'bounds': list(parameter_df.iloc[:,1:3].values)
           }
...
for j in range(len(synthetic_scenario_names)):
    ...
    #samples = pd.read_csv("param_values_13312_sobol_v5.csv", index_col = 0) # filename for ensemble scenarios
...
os.chdir("C:/Users/fere556/Desktop/Sythetic_scenarios/mwd_lower")

colors = [ 'purple', 'violet','navy','blue','deepskyblue','cyan','slategray','tomato',
          'gold','black','limegreen','seagreen']

result = pd.read_csv("param_values_832_sobol_emulator_mwd_lower.csv_sT.csv", index_col = 0)
#result = shortage_sensitivity_s1.copy() # either s1, sT or s2
#names = shortage_sensitivity.columns.values
names = result.index.values
ensemble_nums = 26*np.array([4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048])

synthetic_scenario_names = [] # param_values_26624_sobol_emulator_mwd_lower.csv_sT.csv
for i in range(len(ensemble_nums)):
    synthetic_scenario_names.append("param_values_" +str(ensemble_nums[i])+"_sobol_emulator_mwd_lower.csv_sT.csv")
    
#ensemble_nums = len(synthetic_scenario_names)
for j in range(len(names)):
    ...
    convergence = pd.DataFrame(data = np.zeros((len(ensemble_nums), 12)), 
                                columns = result.columns)
    
    convergence_rankings = pd.DataFrame(data = np.zeros((len(ensemble_nums), 12)), 
                                columns = result.columns)
    
    # ST values 
    for i in range(len(ensemble_nums)):
        result = pd.read_csv(synthetic_scenario_names[i], index_col = 0)
        convergence.iloc[i,:] = result.loc[grp_name]
      
        # Parameter rankings
        sorted_vals = convergence.iloc[i,:].sort_values(ascending = True)
        sorted_vals = pd.DataFrame(data = sorted_vals.values, columns = ['ST'], index = sorted_vals.index)
        sorted_vals['rank'] = np.arange(12)+1
        
        for k in range(len(result.columns)):
            rank = sorted_vals.loc[result.columns[k]]
            convergence_rankings[result.columns[k]][i] = rank.values[1]
    
    fig, ax = plt.subplots()
    for i in range(len(result.columns)):
        ax.plot(26*np.array([4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048]), convergence.iloc[:,i], label = result.columns[i], color = colors[i])
    ...
    fig, ax = plt.subplots()
    for i in range(len(result.columns)):
        ax.plot(26*np.array([4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048]), convergence_rankings.iloc[:,i], label = result.columns[i], color = colors[i])
